...
## Phase 3: DNAT & Configuration Support [checkpoint: 31dd4d6]
- [x] Task: Implement DNAT Logic in C (312caa9)
    - [x] IPv4 Destination IP/Port 변환 및 포트 포워딩 로직 구현
- [x] Task: Implement YAML Configuration Loader (4cac76d)
    - [x] `slog`와 `trace`를 사용한 안정적인 설정 로더 구현 (YAML 기반)
- [x] Task: Write Tests for DNAT & Config (e0c22ed)
    - [x] 설정 파일 기반의 포트 포워딩 동작 검증 테스트 작성
- [x] Task: Implement DNAT & Config to Pass Tests (31dd4d6)
    - [x] 위 테스트를 통과하도록 최종 구현 완료
- [x] Task: Conductor - User Manual Verification 'Phase 3: DNAT & Configuration Support' (Protocol in workflow.md)
